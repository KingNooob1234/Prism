    ...
    return (
        <div className="relative h-screen w-full bg-black overflow-hidden">
            ...
            <div className="absolute top-0 left-0 w-full bg-gradient-to-b from-black/80 via-black/40 to-transparent z-40 flex justify-center items-center space-x-6 text-white/90 font-medium tracking-wide pointer-events-none"
                ...
                <button className={`relative flex flex-col items-center pointer-events-auto transition-opacity ${activeTab === 'following' ? 'opacity-100 font-bold' : 'opacity-50'}`} onClick={() => { if (!user) { base44.auth.redirectToLogin(); return; } setActiveTab('following'); }}>
                    Following
            ...
            >
                ...
                {!isLoading && posts.length === 0 && (
                    <div className="h-screen w-full flex flex-col items-center justify-center gap-4 text-white/40 px-8 text-center">
                        ...
                        {activeTab === 'following' && !user && <button onClick={() => base44.auth.redirectToLogin()} className="px-6 py-3 bg-white text-black font-semibold rounded-2xl text-sm">Sign In</button>}
                    </div>
                )}
                {posts.map((post, i) => (
